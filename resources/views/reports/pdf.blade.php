...
@php
    ...
    $totalOf = function ($item) {
        return (float) ($item->grand_total ?? $item->jumlah_bayar ?? 0);
    };
...
<table class="summary-grid">
    <tr>
        ...
        <td class="summary-card" width="25%"><div class="summary-label">Total Nilai</div><div class="summary-value">{{ $money($totalAmount) }}</div></td>
        <td class="summary-card" width="25%"><div class="summary-label">Status</div><div class="summary-value">{{ $transactions->groupBy('status')->count() }}</div></td>
        <td class="summary-card" width="25%"><div class="summary-label">Gudang</div><div class="summary-value">{{ $transactions->map(function ($item) { return optional($item->gudang)->nama_gudang; })->filter()->unique()->count() }}</div></td>
    </tr>
</table>
...
@else
    ...
    @elseif($exportType === 'kunjungan')
        <table class="compact">
            ...
            <tbody>@foreach($transactions as $item) @php $rows = $itemsOf($item); @endphp @forelse($rows as $idx => $detail)<tr>@if($idx === 0)<td rowspan="{{ $rows->count() }}">{{ $loop->parent->iteration }}</td><td rowspan="{{ $rows->count() }}">{{ $number($item) }}</td><td rowspan="{{ $rows->count() }}">{{ $dateTime($item) }}</td><td rowspan="{{ $rows->count() }}">{{ $item->sales_nama ?? optional($item->user)->name ?? '-' }}</td><td rowspan="{{ $rows->count() }}">{{ $contactName($item) }}<br><span class="muted">{{ $phone($item) }}</span></td><td rowspan="{{ $rows->count() }}">{{ optional($item->gudang)->nama_gudang ?? '-' }}</td><td rowspan="{{ $rows->count() }}">{{ $item->tujuan ?? '-' }}</td><td rowspan="{{ $rows->count() }}"><span class="{{ $statusBadge($item->status ?? null) }}">{{ $item->status ?? '-' }}</span></td>@endif<td>{{ optional($detail->produk)->nama_produk ?? '-' }}<br><span class="muted">{{ optional($detail->produk)->kategori ?? '' }}</span></td><td class="text-center">{{ $qtyOf($detail) }}</td>@if($idx === 0)<td rowspan="{{ $rows->count() }}">{{ $item->memo ?? '-' }}<br><span class="muted">{{ $attachmentText($item) }}</span></td>@endif</tr>@empty<tr><td>{{ $loop->iteration }}</td><td>{{ $number($item) }}</td><td>{{ $dateTime($item) }}</td><td>{{ $item->sales_nama ?? optional($item->user)->name ?? '-' }}</td><td>{{ $contactName($item) }}<br>{{ $phone($item) }}</td><td>{{ optional($item->gudang)->nama_gudang ?? '-' }}</td><td>{{ $item->tujuan ?? '-' }}</td><td>{{ $item->status ?? '-' }}</td><td>-</td><td>-</td><td>{{ $item->memo ?? '-' }}<br>{{ $attachmentText($item) }}</td></tr>@endforelse @endforeach</tbody>
        </table>
    ...
    @else
        <table class="compact">
            ...
            <tbody>@foreach($transactions as $item) @php $rows = $itemsOf($item); @endphp @forelse($rows as $idx => $detail)<tr>@if($idx === 0)<td rowspan="{{ $rows->count() }}">{{ $loop->parent->iteration }}</td><td rowspan="{{ $rows->count() }}">{{ $item->type ?? '-' }}</td><td rowspan="{{ $rows->count() }}">{{ $number($item) }}</td><td rowspan="{{ $rows->count() }}">{{ $dateTime($item) }}</td><td rowspan="{{ $rows->count() }}">{{ $item->sales_nama ?? optional($item->user)->name ?? '-' }}</td><td rowspan="{{ $rows->count() }}">{{ $contactName($item) }}<br><span class="muted">{{ $phone($item) }}</span></td><td rowspan="{{ $rows->count() }}">{{ optional($item->gudang)->nama_gudang ?? '-' }}</td><td rowspan="{{ $rows->count() }}"><span class="{{ $statusBadge($item->status ?? null) }}">{{ $item->status ?? '-' }}</span></td>@endif<td>{{ optional($detail->produk)->nama_produk ?? $detail->kategori ?? $detail->deskripsi ?? '-' }}<br><span class="muted">{{ $detail->deskripsi ?? optional($detail->produk)->kategori ?? '' }}</span></td><td class="text-center">{{ $qtyOf($detail) }}</td><td class="text-right">{{ $money($detail->harga_satuan ?? $detail->jumlah ?? $lineAmountOf($detail)) }}</td>@if($idx === 0)<td rowspan="{{ $rows->count() }}">{{ $item->tax_percentage ?? 0 }}%</td><td rowspan="{{ $rows->count() }}" class="text-right"><strong>{{ $money($totalOf($item)) }}</strong></td><td rowspan="{{ $rows->count() }}">{{ $item->memo ?? $item->keterangan ?? '-' }}<br><span class="muted">{{ $attachmentText($item) }}</span></td>@endif</tr>@empty<tr><td>{{ $loop->iteration }}</td><td>{{ $item->type ?? '-' }}</td><td>{{ $number($item) }}</td><td>{{ $dateTime($item) }}</td><td>{{ $item->sales_nama ?? optional($item->user)->name ?? '-' }}</td><td>{{ $contactName($item) }}<br>{{ $phone($item) }}</td><td>{{ optional($item->gudang)->nama_gudang ?? '-' }}</td><td>{{ $item->status ?? '-' }}</td><td>-</td><td>-</td><td>-</td><td>{{ $item->tax_percentage ?? '-' }}</td><td class="text-right">{{ $money($totalOf($item)) }}</td><td>{{ $item->memo ?? $item->keterangan ?? '-' }}<br>{{ $attachmentText($item) }}</td></tr>@endforelse @endforeach</tbody>
        </table>
    @endif
    ...
    <div class="section">
        ...
        <table class="compact">
            ...
            <tbody>@foreach($transactions->groupBy(function ($item) { return $item->status ?: '-'; }) as $status => $group)<tr><td>{{ $status }}</td><td class="text-right">{{ $group->count() }}</td><td class="text-right">{{ $money($group->sum(function ($item) use ($totalOf) { return $totalOf($item); })) }}</td></tr>@endforeach</tbody>
        </table>
    </div>

    @if($exportType === 'all')
        <div class="section"><h2>Ringkasan Tipe Transaksi</h2><table class="compact"><thead><tr><th>Tipe</th><th class="text-right">Jumlah Data</th><th class="text-right">Total Nilai</th></tr></thead><tbody>@foreach($transactions->groupBy(function ($item) { return $item->type ?: '-'; }) as $type => $group)<tr><td>{{ $type }}</td><td class="text-right">{{ $group->count() }}</td><td class="text-right">{{ $money($group->sum(function ($item) use ($totalOf) { return $totalOf($item); })) }}</td></tr>@endforeach</tbody></table></div>
    @endif
    @if($exportType === 'biaya')
        <div class="section"><h2>Ringkasan Jenis Biaya</h2><table class="compact"><thead><tr><th>Jenis Biaya</th><th class="text-right">Jumlah Data</th><th class="text-right">Total Nilai</th></tr></thead><tbody>@foreach($transactions->groupBy(function ($item) { return $item->jenis_biaya ?: '-'; }) as $jenis => $group)<tr><td>{{ ucfirst($jenis) }}</td><td class="text-right">{{ $group->count() }}</td><td class="text-right">{{ $money($group->sum(function ($item) use ($totalOf) { return $totalOf($item); })) }}</td></tr>@endforeach</tbody></table></div>
    @endif
    @if($exportType === 'kunjungan')
        <div class="section"><h2>Ringkasan Tujuan Kunjungan</h2><table class="compact"><thead><tr><th>Tujuan</th><th class="text-right">Jumlah Kunjungan</th></tr></thead><tbody>@foreach($transactions->groupBy(function ($item) { return $item->tujuan ?: '-'; }) as $tujuan => $group)<tr><td>{{ $tujuan }}</td><td class="text-right">{{ $group->count() }}</td></tr>@endforeach</tbody></table></div>
    @endif
@endif
