...
class ViewKunjungan extends ViewRecord
{
    ...
    protected function getHeaderActions(): array
    {
        ...
        return [
                    ...
                    if ($isPromo && $record->items->isNotEmpty()) {
                        try {
                            DB::transaction(function () use ($record, $user): void {
                                $record->load('items');
                                $this->decrementPromoStock($record);
                            ...
                            Notification::make()
                                ->title('Kunjungan di-approve. Stok promo telah dikurangi.')
                                ->success()
                                ->send();
                        } catch (\Throwable $e) {
            ...
            Action::make('cancel')
                ...
                ->action(function () use ($record) {
                    ...
                    try {
                        ...
                        if ($isPromo && $record->status === 'Approved' && $record->items->isNotEmpty()) {
                            $record->load('items');
                            $this->incrementPromoStock($record);
                        }
            ...
            // ===== DELETE =====
            DeleteAction::make()->visible(fn (): bool => $user->isSuperAdmin() && TransactionDeleteGuard::canDeleteKunjungan($record)),
        ];
    }
    ...
    private function mutatePromoStock(Kunjungan $kunjungan, bool $decrement): void
    {
        ...
        $service = app(InventoryMutationService::class);
        ...
        foreach ($items as $item) {
            ...
            if ($decrement) {
                $service->decrement((int) $kunjungan->gudang_id, (int) $item->produk_id, $quantity, $stockType, $context);
            } else {
                $service->increment((int) $kunjungan->gudang_id, (int) $item->produk_id, $quantity, $stockType, $context);
            }
        }
    }
}
