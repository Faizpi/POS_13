...
@endphp
<table>
    <thead><tr><th>No</th><th>Nomor Pembayaran</th><th>Tanggal</th><th>Invoice Pembelian</th><th>Tanggal Invoice</th><th>Supplier</th><th style="mso-number-format:'\@'">No Telepon</th><th>Pembuat</th><th>Approver</th><th>Gudang</th><th>Metode</th><th>Jumlah Bayar</th><th>Grand Total Invoice</th><th>Status Invoice</th><th>Bukti Bayar</th><th>Keterangan</th><th>Status Pembayaran</th><th>Dibuat</th></tr></thead>
    <tbody>
        @foreach($transactions as $i => $p)
            @php $pembelian = data_get($p, 'pembelian'); @endphp
            <tr><td>{{ $i + 1 }}</td><td>{{ $p->nomor ?? $p->custom_number ?? $p->number ?? '-' }}</td><td>{{ $formatDate($p->tgl_pembayaran) }}</td><td>{{ optional($pembelian)->nomor ?? optional($pembelian)->custom_number ?? '-' }}</td><td>{{ $formatDate(optional($pembelian)->tgl_transaksi) }}</td><td>{{ optional($pembelian)->kontak?->nama ?? $p->display_contact_name ?? '-' }}</td><td style="mso-number-format:'\@'">{{ receipt_format_phone(optional($pembelian)->kontak?->no_telp ?? $p->no_telp_kontak ?? '') }}</td><td>{{ optional($p->user)->name ?? '-' }}</td><td>{{ optional($p->approver)->name ?? '-' }}</td><td>{{ optional($p->gudang)->nama_gudang ?? optional(optional($pembelian)->gudang)->nama_gudang ?? '-' }}</td><td>{{ $p->metode_pembayaran ?? '-' }}</td><td>{{ format_rupiah($p->jumlah_bayar ?? 0) }}</td><td>{{ optional($pembelian)->grand_total !== null ? format_rupiah(optional($pembelian)->grand_total) : '-' }}</td><td>{{ optional($pembelian)->status ?? '-' }}</td><td>{{ $p->bukti_bayar ? basename($p->bukti_bayar) : '-' }}</td><td>{{ $p->keterangan ?? '-' }}</td><td>{{ $p->status ?? '-' }}</td><td>{{ $formatDate($p->created_at, 'd/m/Y H:i') }}</td></tr>
        @endforeach
    </tbody>
...
<table>
    <thead><tr><th colspan="4">RINGKASAN PEMBAYARAN HUTANG</th></tr></thead>
    <tbody>
