        ...
        return [
            ...
            EditAction::make()
                ...
                ->visible(function () use ($user, $record): bool {
                    ...
                    if ($user->role === 'admin') return $user->canAccessGudang($record->gudang_id);
                    return false;
                }),
            ...
            Action::make('approve')
                ...
                ->action(function () use ($record, $user): void {
                    ...
                    DB::transaction(function () use ($record, $user): void {
                        ...
                        $totalBayar = Pembayaran::where('penjualan_id', $record->penjualan_id)
                            ->where('status', 'Approved')
                            ->sum('jumlah_bayar');

                        $penjualan = $record->penjualan;
                        if ($penjualan && (float) $totalBayar >= (float) $penjualan->grand_total) {
            ...
            Action::make('uncancel')
                ...
                ->action(function () use ($record): void {
                    $record->update(['status' => 'Pending', 'approver_id' => auth()->id()]);
                    Notification::make()
            ...
            DeleteAction::make()
                ...
                ->color('danger')
                ->requiresConfirmation()
                ->visible(fn(): bool => $user->isSuperAdmin()),
        ];
    }
}
