<x-filament-panels::page>
    ...
    <div class="space-y-4">
        ...
        <dl class="grid gap-3 sm:grid-cols-4">
            ...
            <div class="rounded-lg border p-3"><dt>Mutasi debit</dt><dd class="font-semibold">{{ $data['movement_debit'] }}</dd></div>
            <div class="rounded-lg border p-3"><dt>Mutasi kredit</dt><dd class="font-semibold">{{ $data['movement_credit'] }}</dd></div>
            <div class="rounded-lg border p-3"><dt>Saldo akhir</dt><dd class="font-semibold">{{ $data['ending_balance'] }}</dd></div>
        </dl>

        <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
            <table class="w-full text-sm">
                <thead><tr class="border-b text-left"><th class="p-3">Tanggal</th><th class="p-3">Nomor</th><th class="p-3">Sumber</th><th class="p-3 text-right">Debit</th><th class="p-3 text-right">Kredit</th><th class="p-3 text-right">Saldo</th></tr></thead>
                <tbody>
                    @forelse ($data['rows'] as $row)
                        <tr class="border-b"><td class="p-3">{{ $row['journal_date'] }}</td><td class="p-3">{{ $row['journal_number'] }}</td><td class="p-3">{{ $row['source_type'] }} #{{ $row['source_id'] }}</td><td class="p-3 text-right">{{ $row['debit'] }}</td><td class="p-3 text-right">{{ $row['credit'] }}</td><td class="p-3 text-right">{{ $row['running_balance'] }}</td></tr>
                    @empty
                        <tr><td colspan="6" class="p-3 text-center text-gray-500">Pilih akun untuk melihat Buku Besar.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
